...
class ModifiedResNet18_with_true_8_layers(nn.Module):
    def __init__(self, num_classes=1000, num_hidden=64, layers=[1,1,2,1,1,2,1,1]):
        super(ModifiedResNet18_with_true_8_layers, self).__init__()
        ...
        self.layer1 = self._make_layer(block=BasicBlock, out_channels=num_hidden*2, blocks=layers[0],stride=1)
        self.layer2 = self._make_layer(block=BasicBlock, out_channels=num_hidden*2, blocks=layers[1], stride=2)
        self.layer3 = self._make_layer(block=BasicBlock, out_channels=num_hidden*3, blocks=layers[2], stride=1)
        self.layer4 = self._make_layer(block=BasicBlock, out_channels=num_hidden*4, blocks=layers[3], stride=2)
        ...
        self.layer7 = self._make_layer(block=BasicBlock, out_channels=num_hidden*8, blocks=layers[6], stride=1)
        self.layer8 = self._make_layer(block=BasicBlock, out_channels=num_hidden*8, blocks=layers[7], stride=2)
...
model = ModifiedResNet18_with_true_8_layers(10, 32)
...
print("Summary second time", torchsummary.summary(model, input_size =(3,32,32), device='cpu'))
